...
def lbfgs(g, s, y): 
    ...
    # al = np.zeros((k,1))
    # be = np.zeros((k,1))
    q = torch.zeros(p, k+1, device=device, dtype=dtype)
    r = torch.zeros(p, k+1, device=device, dtype=dtype)
    al = torch.zeros(k, device=device, dtype=dtype)
    be = torch.zeros(k, device=device, dtype=dtype)
    q[:,k] = g
