...
#    record[:,5]
#    F_star = min(min(record_all[0][:,0]), min(record_all[1][:,0]),
#                 min(record_all[2][:,0]), min(record_all[3][:,0]))
...
#        record[:,0] = (record[:,0] - F_star)/max(F_star, 1)
    ...
    plt.xlabel('Oracle call', fontsize=fsize)
    plt.ylabel(r'$ f(x_k) $', fontsize=fsize)
...
#    plt.ylabel(r'$\frac{f(x_k) - f^{*}}{\max \{f^{*}, 1\}}$', fontsize=fsize)
    ...
    plt.ylabel(r'$ \alpha $ or $ \Delta $', fontsize=fsize)
