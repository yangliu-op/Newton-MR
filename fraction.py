...
import torch
def fraction(x, a=100, b=1, arg = None, HProp = None):
    """
    Fraction problems: 
        F(x, y): a*x^2/(b - y)
    INPUT:
...
#    print(a, b, x2)
    ...
    if arg is None:
        H23 = 2*x1*H2
        H = torch.tensor([[-2*H1, H23],[H23, -2*x1**2*H3]])
        if HProp == None:
            return f, g, H
        else:
            s = svd(H)[1]
...
def main():    
    obj = lambda w, arg=None: fraction(w,a=2,b=1,arg=arg)
    x0 = torch.randn(2, dtype=torch.float64)
